...
program
    ...
    for (const name of names) {
        try {
            ...
            console.error(`❌ Failed to switch ${name}:`, e instanceof Error ? e.message : e);
        }
    }
    process.exit(0);
});
...
program
    .command("update")
    .description("Refresh the current version of the libraries (keeps current local/remote state)")
    ...
    for (const name of targets) {
        try {
            ...
            console.error(`❌ Failed to update ${name}:`, e instanceof Error ? e.message : e);
        }
    }
    process.exit(0);
});
